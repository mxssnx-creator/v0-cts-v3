...
export class PositionCalculator {
  ...
  calculateSymbolPositions(symbol: string): SymbolAnalysis {
    ...
    // Partial Strategies
    const partialStrategies = this.calculateStrategyConfigs("partial")
    categories.push({
      category: "Strategies",
      subcategory: "Partial Strategy",
      configurations: partialStrategies.configs,
      positions_per_config: 1, // Max 1 active position per configuration
      total_positions: partialStrategies.configs * 1,
      limit_applied: 1,
      description: `Won/Loss analysis with ${partialStrategies.position_counts} position count variations (1 active per config)`,
    })

    // Count Strategies (2, 4 positions)
    const countStrategies = this.calculateStrategyConfigs("count")
    categories.push({
      category: "Strategies",
      subcategory: "Count Strategy",
      configurations: countStrategies.configs,
      positions_per_config: 1, // Max 1 active position per configuration
      total_positions: countStrategies.configs * 1,
      limit_applied: 1,
      description: `2 count variations (2,4) × ${countStrategies.base_configs} base configurations (1 active per config)`,
    })
    ...
    return {
      ...
      summary: {
        ...
        strategy_types: 5, // Base, Partial, Count, Block, DCA
  ...
  private calculateStrategyConfigs(type: "base" | "partial" | "count" | "block" | "dca") {
    ...
    switch (type) {
      ...
      case "partial":
        return {
          position_counts: 3, // Different partial position counts
