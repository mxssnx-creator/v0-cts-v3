...
export async function GET(request: NextRequest) {
  try {
    ...
    const validatedPresets = presets.map((preset: any) => ({
      ...
      strategy_types: preset.strategy_types ? JSON.parse(preset.strategy_types) : ["base", "partial", "count"],
      last_positions_counts: preset.last_positions_counts
        ? JSON.parse(preset.last_positions_counts)
        : [3, 4, 5, 6, 8, 12, 25],
      partial_positions_counts: preset.partial_positions_counts
        ? JSON.parse(preset.partial_positions_counts)
        : [1, 2, 3, 4, 5],
  ...
  try {
    ...
    await execute(
      `INSERT INTO presets (
        ...
        last_positions_counts, partial_positions_counts,
      ...
      [
        ...
        JSON.stringify(body.strategy_types || ["base", "partial", "count"]),
        JSON.stringify(body.last_positions_counts || [3, 4, 5, 6, 8, 12, 25]),
        JSON.stringify(body.partial_positions_counts || [1, 2, 3, 4, 5]),
