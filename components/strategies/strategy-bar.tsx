...
export function StrategyBar({ strategy, onToggle, onVolumeFactorChange, minimalProfitFactor }: StrategyBarProps) {
  ...
  const getStrategyIcon = (name: string) => {
    ...
    if (name.includes("Partial")) return <Activity className="h-4 w-4" />
    if (name.includes("Count")) return <BarChart3 className="h-4 w-4" />
  ...
  const getStrategyColor = (name: string) => {
    ...
    if (name.includes("Partial")) return "bg-green-100 text-green-800"
    if (name.includes("Count")) return "bg-purple-100 text-purple-800"
